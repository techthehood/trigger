  ...
  const {obj_exists, exists} = require('../../tools/exists.js');
  ...
  const ProfileIcon = (props) => {
    ...
    const {
      // data = exists(props.data) ? props.data : props;
      item = {},
      no_class = false,
      preset_data,// LATER: remove preset_data
      image,
      default_image,
      variant = "",
      // extra,// use props.extra
      name =  "",
      tag =  "",
      fallback =  "icon",
    ...
    // let image = "https://lh3.googleusercontent.com/a/AATXAJwgMXS7tol73JbwAmqho17E2cfNL_rg7_faPsRUX=s96-c";// broken image
    ...
    */
    ...
    let last_image = useRef(image);
    const unmounting = useRef(false);
    // set the image stage
    if(init.current == false || last_image.current != image){
      // determine the initial state
      if(exists(image) && image != "unavailable"){
        // were in stage 1, continue
      }else if(exists(default_image) && default_image != "unavailable"){
        // default state
      ...
      last_image.current = image;
    }// init

    // if no callback is provided && deep_dive == true, runs go_to_profile
    let deep_dive = (exists(deep_dive)) ? deep_dive : false;

    let iUN_ref = useRef(Math.round(Math.random() * 10000));
    let iUN = props.iUN || iUN_ref.current;

    const fresh_val = useRef(0);
    const timer = useRef();
    ...
    const img_error = (obj) => {
      ...
      // }//if
      if(imageStage.current == 1 && 
        exists(default_image) && default_image != "unavailable"){
      ...
      try {
        // const fresh_val.current = val;// i may not need this
        timer.current = setTimeout(() => {
          console.log(`[ProfileIcon] running timeout`);
          // forceUpdate();// GOTCHA: this breaks the entire app - can't force update b/c img_error is running in ITKR useEffect > draw_image
          // clearTimeout(timer.current);
        }, 2000);
      } catch (error) {
        console.error(`[ProfileIcon] img_error forceUpdate`,error);
      }
    }// img_error
    ...
    let image_item = (imageStage.current == 1) ? image :
    (imageStage.current == 2) ? default_image :
    null;

    let extra = (exists(props.extra)) ? has_extra_data({extra: props.extra, layout: "profile"}, true) : null;// i need the profile extra or use the default
    ...
    let icon_image = (exists(image_item)) ? image_item :  get_default_image();
    ...
    if (accept_image) {
      ...
      let img_data = {
        ...
        url:icon_image,
        type: "profile",
        custom_class:`${my_icon_str} ${variant} ${/*s.app_state.text_view*/""}`,
        canvas_class: no_class ? "" : `d3-profile-rounded`,
        image_class: no_class ? "" : `d3-profile-rounded`,
        width:100,height:100,scale:my_scale,
        mode:"image",
        stop_bubble: true,
        auto_validate: true,// avoid auto_validate; the default_image is an issue in the itkr
        ...
        default_img: window['IMG_URL'] + "flame.png",// change this to user icon img
      };
