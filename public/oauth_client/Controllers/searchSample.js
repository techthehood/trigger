...
const searchSample = async function (req, res) {
  try {

    // let processed_msg = `[searchSample] searchSample api accessed`;
    ...
    // if (display_console || true) console.log(chalk.keyword('aquamarine')("[searchSample] body"), req.body);
    // // throw "testing stack";
    ...
    if (display_console || true) console.log(chalk.green("[searchSample] body "), req.body);
    if (display_console || true) console.log(chalk.yellow("[searchSample] user "), req.user);
    ...
    if (row_more) {
      ...
      rows = rows.map((row) => {
        ...
        if (typeof row.username != "undefined") ready_row.username;
    ...
    let err_msg = "[searchSample] an error occured";
